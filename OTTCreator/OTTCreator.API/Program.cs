...
app.MapGet("contentitems", async (ApplicationIdentityDbContext db) =>
    await db.ContentItems.ToListAsync());

app.MapGet("types", async (ApplicationIdentityDbContext db) =>
    await db.ContentItems.Select(c => c.Type).Distinct().ToListAsync());

app.MapGet("{type}/categories/", async (string type, ApplicationIdentityDbContext db) =>
    await db.ContentItems.Where(c => c.Type == type).Select(c => c.Category).Distinct().ToListAsync());

app.MapGet("{type}/{category}/contentitems", async (string type, string category, ApplicationIdentityDbContext db) =>
    await db.ContentItems.Where(c => c.Type == type && c.Category == category).ToListAsync());

app.MapGet("types/favorites", async (ApplicationIdentityDbContext db) =>
    await db.ContentItems.Where(c => c.IsFavorite).Select(c => c.Type).ToListAsync());

app.MapGet("types/recommended", async (ApplicationIdentityDbContext db) =>
    await db.ContentItems.Where(c => c.IsRecommended).Select(c => c.Type).ToListAsync());

app.MapGet("{type}/contentitems/favorites", async (string type, ApplicationIdentityDbContext db) =>
    await db.ContentItems.Where(c => c.Type == type && c.IsFavorite == true).ToListAsync());

app.MapGet("{type}/contentitems/recommended", async (string type, ApplicationIdentityDbContext db) =>
    await db.ContentItems.Where(c => c.Type == type && c.IsRecommended == true).ToListAsync());

app.MapGet("contentitems/{id}", async (int id, ApplicationIdentityDbContext db) =>
    await db.ContentItems.FindAsync(id)
        is ContentItem contentItem
            ? Results.Ok(contentItem)
            : Results.NotFound());

app.MapPut("contentitems/{id}/favorite", async (int id, ApplicationIdentityDbContext db) =>
{
